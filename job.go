...
func (j *Job) Invoke(ctx context.Context, files []string, skipIfRunning bool, skipIfQueued bool, params map[string]string, cause string, securityToken string) (int64, error) {
	if skipIfQueued {
		isQueued, err := j.IsQueued(ctx)
		if err != nil {
			return 0, err
